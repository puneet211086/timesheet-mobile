...
export function useReports() {
  ...
  const range = useMemo(() => {
    const start = period === 'week' ? startOfWeek(anchorDate) : startOfMonth(anchorDate);
    const end = period === 'week' ? endOfWeek(anchorDate) : endOfMonth(anchorDate);
    return { start, end };
  }, [anchorDate, period]);
  ...
  const resetToCurrentPeriod = useCallback(() => setAnchorDate(new Date()), []);
