...
export default function SettingsScreen() {
  ...
  const toggleReminder = async (nextValue: boolean) => {
    if (Platform.OS === "web") {
      Alert.alert(
        "Mobile feature",
        "Local shift reminders are available on iOS and Android.",
      );
      return;
    }
    ...
    if (nextValue) {
      ...
      if (!granted) {
        Alert.alert(
          ...
          "Enable notifications for this app in your device settings to use shift reminders.",
        );
        return;
      }
    }
    ...
    try {
      ...
      if (!sent) {
        Alert.alert(
          ...
          Platform.OS === "web"
            ? "Test notifications are available on iOS and Android."
    ...
    <SafeAreaView style={styles.safeArea}>
      <ScrollView contentContainerStyle={styles.container}>
        {Platform.OS === "web" && (
          <View style={styles.webNotice}>
            ...
            <Text style={styles.webNoticeText}>
              Notification settings are available when testing the app on an iOS
              or Android device.
            </Text>
          </View>
        )}
        ...
        <Text style={styles.subtitle}>
          Configure reminders that help prevent forgotten clock-outs.
        </Text>
        ...
        <View style={styles.card}>
          <View style={styles.settingRow}>
            ...
            <View style={styles.settingCopy}>
              ...
              <Text style={styles.settingDescription}>
                Notify me when an active shift reaches the selected duration.
              </Text>
            </View>
            <Switch
              value={enabled}
            ...
            {REMINDER_OPTIONS.map((option) => {
              ...
              return (
                <Pressable
                  key={option}
                  disabled={!enabled}
                  onPress={() => void chooseHours(option)}
                  style={[
                    styles.option,
          ...
          <Pressable
            disabled={testing || Platform.OS === "web"}
            onPress={() => void testReminder()}
            style={({ pressed }) => [
              styles.testButton,
              pressed && styles.pressed,
              (testing || Platform.OS === "web") && styles.testButtonDisabled,
            ]}
          >
            {testing ? (
        ...
        <View style={styles.infoCard}>
          ...
          <Text style={styles.infoText}>
            Reminders are scheduled locally on your phone when you clock in and
            are canceled when you clock out. No account or cloud service is
            required.
          </Text>
        </View>
      </ScrollView>
...
const styles = StyleSheet.create({
  ...
  title: { color: "#111827", fontSize: 32, fontWeight: "800", marginTop: 5 },
  subtitle: {
    color: "#64748B",
    fontSize: 16,
    lineHeight: 23,
  ...
  settingTitle: { color: "#0F172A", fontSize: 16, fontWeight: "800" },
  settingDescription: {
    color: "#64748B",
    fontSize: 13,
    lineHeight: 19,
  ...
  testButtonText: { color: "#2563EB", fontSize: 15, fontWeight: "800" },
  ...
  infoText: { flex: 1, color: "#475569", fontSize: 13, lineHeight: 19 },
  webNotice: {
    flexDirection: "row",
    alignItems: "center",
    gap: 10,
