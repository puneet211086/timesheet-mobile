      ...
      <ScrollView contentContainerStyle={styles.container}>
        ...
        {todayEntries.length === 0 ? (
          <View style={styles.emptyCard}>
            <View style={styles.emptyIcon}><Ionicons name="time-outline" size={28} color={colors.primary} /></View>
              ...
              <View style={styles.entryBody}>
                <View style={styles.entryTopRow}>
                  <View style={styles.entryText}>
                    ...
                    <Text style={styles.time}>{formatClockTime(entry.clockIn)} – {entry.clockOut ? formatClockTime(entry.clockOut) : 'Running'}</Text>
                  </View>
                ...
                {entry.unpaidBreakMinutes > 0 ? (
                  <Text style={styles.breakText}>{entry.unpaidBreakMinutes} min unpaid break deducted</Text>
                ) : null}
                {entry.notes ? <Text style={styles.notes}>{entry.notes}</Text> : null}
                <Text style={editable ? styles.editHintText : styles.runningHint}>
                  {editable ? 'Edit entry' : 'Clock out before editing'}
                </Text>
              </View>
            </Pressable>
          );
