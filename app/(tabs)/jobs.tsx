  ...
  const submit = async () => {
    ...
    if (!Number.isFinite(overtimeMultiplier) || overtimeMultiplier < 1) {
      Alert.alert(
        ...
        "Use a multiplier of 1.0 or higher.",
      );
      return;
    }
    ...
    setSaving(true);
    try {
      await saveJob({ ...form, hourlyRate, overtimeMultiplier }, editingId);
  ...
  const confirmToggle = (job: Job) => {
    ...
    const updateStatus = () => {
      setJobActive(job.id, !job.isActive).catch((error) => {
        ...
        if (Platform.OS === "web") {
          window.alert("The job status could not be updated.");
        } else {
          Alert.alert("Update failed", "The job status could not be updated.");
        }
      });
    };

    if (Platform.OS === "web") {
      const confirmed = window.confirm(`${action} ${job.name}?\n\n${message}`);
      ...
      <ScrollView contentContainerStyle={styles.container}>
        ...
        <View style={styles.list}>
          {jobs.map((job) => (
            <View
              key={job.id}
              style={[styles.card, !job.isActive && styles.cardInactive]}
            >
              ...
              <View style={styles.cardContent}>
                ...
                <View style={styles.metricsRow}>
                  ...
                  <View>
                    ...
                    <Text style={styles.metricValue}>
                      {job.overtimeMultiplier.toFixed(1)}×
                    </Text>
                  </View>
                </View>

                <Pressable
                  onPress={() => confirmToggle(job)}
